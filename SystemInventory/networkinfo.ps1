...
param(
	...
	[ValidateNotNullOrEmpty()]
	[string]
	$BasePath = (Join-Path -Path $DirectoryPath -ChildPath ('NWInventory - ' + (Get-Date -Format 'yyyy-MM-dd-HHmm') + '.csv'))
	,
	[Parameter(Mandatory=$false)] 
	[ValidateNotNullOrEmpty()]
	[string]
...
foreach ($c in $ComputerName) {
	...
	$ComputerSystem = Get-WmiObject -Class Win32_ComputerSystem -ComputerName $c -EV Err -ErrorAction SilentlyContinue
		...
		Else 
		{
			$Networks = Get-WmiObject Win32_NetworkAdapterConfiguration -ComputerName $c | ? {$_.IPEnabled}
			foreach ($Network in $Networks) {
				...
				$reportinfo | Add-Member -MemberType NoteProperty -Name ComputerName -Value $Network.PSComputername
				$reportinfo | Add-Member -MemberType NoteProperty -Name AdapterName -Value $AdapterName
				$reportinfo | Add-Member -MemberType NoteProperty -Name IsDHCPEnabled -Value $IsDHCPEnabled
				$reportinfo | Add-Member -MemberType NoteProperty -Name IPAddress -Value $IPAddress
				$reportinfo | Add-Member -MemberType NoteProperty -Name SubnetMask -Value $SubnetMask
				$reportinfo | Add-Member -MemberType NoteProperty -Name Gateway -Value ($DefaultGateway -join "/")
				$reportinfo | Add-Member -MemberType NoteProperty -Name DNSServers -Value ($DNSServers -join "/")
				$reportinfo | Add-Member -MemberType NoteProperty -Name MACAddress -Value $MACAddress
