...
param(
	...
	[ValidateNotNullOrEmpty()]
	[string]
	#$BasePath = (Join-Path -Path $DirectoryPath -ChildPath ('Contacts - ' + (Get-Date -Format 'yyyy-MM-dd-HHmm') + '.csv'))
	$BasePath = ".\contacts.csv"
	,
	[Parameter(Mandatory=$false)] 
	[ValidateNotNullOrEmpty()]
	[string]
...
Foreach ($c in $ComputerName){
	if(Test-Connection -ComputerName $c -Count 1 -ea 0 -ev Err){ 
		...
		Else 
		{
			...
			$reportinfo | Add-Member -MemberType NoteProperty -name Environment -Value $ID
			$reportinfo | Add-Member -MemberType NoteProperty -name Application -Value $ID
			$reportinfo | Add-Member -MemberType NoteProperty -name ApplicationManager -Value $ID
			$reportinfo | Add-Member -MemberType NoteProperty -name Comment -Value $ID
			$reportinfo | Add-Member -MemberType NoteProperty -name SLA -Value $ID
