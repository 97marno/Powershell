...
param(
	...
	[ValidateNotNullOrEmpty()]
	[string]
	$BasePath = (Join-Path -Path $DirectoryPath -ChildPath ('ServicesInventory - ' + (Get-Date -Format 'yyyy-MM-dd-HH-mm') + '.csv'))
	,
	[Parameter(Mandatory=$false)] 
	[ValidateNotNullOrEmpty()]
	[string]
...
foreach ($c in $ComputerName) {
	...
	$ComputerSystem = Get-WmiObject -Class Win32_ComputerSystem -ComputerName $c -EV Err -ErrorAction SilentlyContinue
		...
		Else 
		{$services = Get-WmiObject win32_Service -ComputerName $c -ErrorAction Stop | Select-Object -property displayName, Name, StartMode, StartName, PathName
			foreach ($s in $services) {
				...
				$reportinfo | Add-Member -MemberType NoteProperty -Name DisplayName -Value $s.displayName
				$reportinfo | Add-Member -MemberType NoteProperty -Name Name -Value $s.Name
				$reportinfo | Add-Member -MemberType NoteProperty -Name StartMode -Value $s.StartMode
				$reportinfo | Add-Member -MemberType NoteProperty -Name StartName -Value $s.StartName
				$reportinfo | Add-Member -MemberType NoteProperty -Name PathName -Value $s.PathName
